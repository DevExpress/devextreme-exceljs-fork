...
const utils = {
  ...
  getRelsPath(filepath) {
    ...
    return `${parsed.path}/_rels/${parsed.name}.rels`;
  },
  assertSafeMediaPath(filename) {
    if (filename === undefined || filename === null) {
      return;
    }
    if (typeof filename !== 'string') {
      throw new Error(`Unsafe media filename: expected a string but received ${typeof filename}`);
    }
    if (filename.includes('\0')) {
      throw new Error(`Unsafe media filename "${filename}": null bytes are not allowed`);
    }
    if (/(^|[\\/:])\.\.($|[\\/:])/.test(filename)) {
      throw new Error(`Unsafe media filename "${filename}": path traversal ("..") segments are not allowed`);
    }
  },
  assertSafeMediaExtension(extension) {
    if (extension === undefined || extension === null) {
      return;
    }
    if (typeof extension !== 'string') {
      throw new Error(`Unsafe media extension: expected a string but received ${typeof extension}`);
    }
    if (extension.includes('\0')) {
      throw new Error(`Unsafe media extension "${extension}": null bytes are not allowed`);
    }
    if (/[\\/]/.test(extension) || extension.includes('..')) {
      throw new Error(`Unsafe media extension "${extension}": path separators and ".." are not allowed`);
    }
  },
  safeJoin(baseDir, userPath) {
