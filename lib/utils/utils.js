...
const utils = {
  ...
  assertSafeMediaExtension(extension) {
    ...
    if (typeof extension !== 'string') {
      throw new Error(`Unsafe image extension: expected a string but received ${typeof extension}`);
    }
    if (/[\\/]/.test(extension) || extension.includes('..')) {
      throw new Error(`Unsafe image extension "${extension}": path separators and ".." are not allowed`);
    }
  },
  xmlEncode(text) {
